...
impl<'a> Deposit<'a> {
    ...
    pub fn process(&mut self) -> ProgramResult {
        ...
        let (seed_bytes, config_bump, locked, mint_x_bytes, mint_y_bytes) = {
        ...
        if self.instruction_data.amount == 0
            || self.instruction_data.max_x == 0
            || self.instruction_data.max_y == 0
        {
            return Err(AmmError::InvalidAmount.into());
        }

        let (vault_x_bal, vault_y_bal) = {
        ...
        let lp_supply = {
            let lp_mint = Mint::from_account_view(a.mint_lp)?;
            let supply = lp_mint.supply();
            if supply == 0 {
        ...
        if x_required > self.instruction_data.max_x || y_required > self.instruction_data.max_y {
        ...
        batch::<3, 9>(
            [
                TokenOp::transfer(a.user_x, a.vault_x, a.user, x_required),
                TokenOp::transfer(a.user_y, a.vault_y, a.user, y_required),
                TokenOp::mint_to(a.mint_lp, a.user_lp, a.config, self.instruction_data.amount),
            ],
            a.token_program,
            &signers,
        )?;
